...
import type { AddOrgCopilotSeatsForTeamsSuccess, AddOrgCopilotSeatsForTeamsFailure } from "../../../types.js";
...
export function registerGithubAddOrgCopilotSeatsForTeamsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_add_org_copilot_seats_for_teams",
        ...
        async (input) => {
            try {
                const response = await octokit.rest.copilot.addCopilotSeatsForTeams({ org: input.org,
                    selected_teams: input.selected_teams });
                const requestId = getRequestId(response.headers["x-github-request-id"]);
                const successPayload: AddOrgCopilotSeatsForTeamsSuccess = {
                    success: true,
                ...
                const failurePayload: AddOrgCopilotSeatsForTeamsFailure = {
                    success: false,
