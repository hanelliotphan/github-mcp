...
export function registerGithubAddResourcesToEnterpriseCostCenterTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_add_resources_to_enterprise_cost_center",
        "Add resources to a cost center (POST /enterprises/{enterprise}/settings/billing/cost-centers/{cost_center_id}/resource). " +
            "Requires enterprise admin. Pass one or more of users, organizations, repositories, enterprise_teams. " +
            "See [Add resources to a cost center](https://docs.github.com/en/rest/billing/cost-centers?apiVersion=2026-03-10#add-resources-to-a-cost-center).",
        {
            enterprise: z
                .string()
                .min(1)
        ...
        async (input) => {
            try {
                ...
                const successPayload: AddResourcesToEnterpriseCostCenterSuccess = {
                    success: true,
                    message: "Resources added to cost center successfully.",
                ...
                const failurePayload: AddResourcesToEnterpriseCostCenterFailure = {
                    success: false,
