...
export function registerGithubListOrgCodeScanningDismissalRequestsTool(
    server: McpServer,
    octokit: Octokit
): void {
    server.tool(
        "github_list_org_code_scanning_dismissal_requests",
        "List dismissal requests for code scanning alerts for an organization (GET /orgs/{org}/dismissal-requests/code-scanning). " +
            "Optional filters: reviewer, requester, time_period, request_status, repository_name. " +
            "Classic tokens need `security_events`. " +
            "See [List dismissal requests for code scanning alerts for an organization](https://docs.github.com/en/rest/code-scanning/alert-dismissal-requests?apiVersion=2026-03-10#list-dismissal-requests-for-code-scanning-alerts-for-an-organization).",
        {
            org: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListOrgCodeScanningDismissalRequestsSuccess = {
                        success: true,
                ...
                const response = await octokit.request("GET /orgs/{org}/dismissal-requests/code-scanning", {
                    org: input.org,
                    ...filters,
                    per_page: perPage,
                    page
                ...
                const failurePayload: ListOrgCodeScanningDismissalRequestsFailure = {
                    success: false,
