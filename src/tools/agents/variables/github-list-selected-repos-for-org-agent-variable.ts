...
export function registerGithubListSelectedReposForOrgAgentVariableTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_selected_repos_for_org_agent_variable",
        "List repositories that can access an organization agent variable whose visibility is `selected` (GET /orgs/{org}/agents/variables/{name}/repositories). " +
            ...
            "See [List selected repositories for an organization variable](https://docs.github.com/en/rest/agents/variables?apiVersion=2026-03-10#list-selected-repositories-for-an-organization-variable).",
        {
            org: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListSelectedReposForOrgAgentVariableSuccess = {
                        success: true,
                ...
                const response = await octokit.request("GET /orgs/{org}/agents/variables/{name}/repositories", {
                    org: input.org,
                    name: input.name,
                    per_page: perPage,
                    page
                ...
                const failurePayload: ListSelectedReposForOrgAgentVariableFailure = {
                    success: false,
