...
export function registerGithubListRepoAgentVariablesTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_repo_agent_variables",
        "List repository GitHub Agents variables (GET /repos/{owner}/{repo}/agents/variables). " +
            ...
            "See [List repository variables](https://docs.github.com/en/rest/agents/variables?apiVersion=2026-03-10#list-repository-variables).",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllPageLinkPages({
                        ...
                        fetchPage: async (page, pp) => {
                            const response = await octokit.request("GET /repos/{owner}/{repo}/agents/variables", {
                                owner: input.owner,
                                repo: input.name,
                                per_page: pp,
                                page
                    ...
                    const successPayload: ListRepoAgentVariablesSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Repository agent variables partially listed (${result.pagesFetched} pages, ${variables.length} rows); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Repository agent variables listed successfully (${result.pagesFetched} pages, ${variables.length} rows).`
                              : "Repository agent variables listed successfully.",
                        owner: input.owner,
                        repo: input.name,
                        full_name,
                ...
                const successPayload: ListRepoAgentVariablesSuccess = {
                    success: true,
                    message: "Repository agent variables listed successfully.",
                    owner: input.owner,
                    repo: input.name,
                    full_name,
                ...
                const failurePayload: ListRepoAgentVariablesFailure = {
                    success: false,
