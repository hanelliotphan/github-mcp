...
/** Default when `per_page` is omitted. GitHub documents max **30** for this endpoint. */
...
export function registerGithubListRepoOrganizationAgentVariablesTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_repo_organization_agent_variables",
        "List organization GitHub Agents variables shared with a repository (GET /repos/{owner}/{repo}/agents/organization-variables). " +
            "Returns **`total_count`** and **`variables`** (name, value, timestamps). " +
            ...
            "See [List repository organization variables](https://docs.github.com/en/rest/agents/variables?apiVersion=2026-03-10#list-repository-organization-variables).",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListRepoOrganizationAgentVariablesSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Repository organization agent variables partially listed (${result.pagesFetched} pages, ${variables.length} rows); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Repository organization agent variables listed successfully (${result.pagesFetched} pages, ${variables.length} rows).`
                              : "Repository organization agent variables listed successfully.",
                        owner: input.owner,
                        repo: input.name,
                        full_name,
                ...
                const response = await octokit.request("GET /repos/{owner}/{repo}/agents/organization-variables", {
                    owner: input.owner,
                    repo: input.name,
                    per_page: perPage,
                    page
                ...
                const successPayload: ListRepoOrganizationAgentVariablesSuccess = {
                    success: true,
                    message: "Repository organization agent variables listed successfully.",
                    owner: input.owner,
                    repo: input.name,
                    full_name,
                ...
                const failurePayload: ListRepoOrganizationAgentVariablesFailure = {
                    success: false,
