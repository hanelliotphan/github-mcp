...
export function registerGithubListOrgAgentVariablesTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_org_agent_variables",
        "List GitHub Agents variables in an organization (GET /orgs/{org}/agents/variables). " +
            ...
            "See [List organization variables](https://docs.github.com/en/rest/agents/variables?apiVersion=2026-03-10#list-organization-variables).",
        {
            org: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllPageLinkPages({
                        ...
                        fetchPage: async (page, pp) => {
                            const response = await octokit.request("GET /orgs/{org}/agents/variables", {
                                org: input.org,
                                per_page: pp,
                                page
                            } as never);
                    ...
                    const successPayload: ListOrgAgentVariablesSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Organization agent variables partially listed (${result.pagesFetched} pages, ${variables.length} rows); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Organization agent variables listed successfully (${result.pagesFetched} pages, ${variables.length} rows).`
                              : "Organization agent variables listed successfully.",
                        org: input.org,
                ...
                const successPayload: ListOrgAgentVariablesSuccess = {
                    success: true,
                    message: "Organization agent variables listed successfully.",
                    org: input.org,
                ...
                const failurePayload: ListOrgAgentVariablesFailure = {
                    success: false,
