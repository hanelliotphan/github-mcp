...
export function registerGithubListRepoOrganizationAgentSecretsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_repo_organization_agent_secrets",
        "List organization GitHub agent secrets shared with a repository, without revealing their values (GET /repos/{owner}/{repo}/agents/organization-secrets). " +
            ...
            "See [List repository organization secrets](https://docs.github.com/en/rest/agents/secrets?apiVersion=2026-03-10#list-repository-organization-secrets).",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListRepoOrganizationAgentSecretsSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Repository organization agent secrets partially listed (${result.pagesFetched} pages, ${secrets.length} rows); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Repository organization agent secrets listed successfully (${result.pagesFetched} pages, ${secrets.length} rows).`
                              : "Repository organization agent secrets listed successfully.",
                        owner: input.owner,
                        repo: input.name,
                        full_name,
                ...
                const successPayload: ListRepoOrganizationAgentSecretsSuccess = {
                    success: true,
                    message: "Repository organization agent secrets listed successfully.",
                    owner: input.owner,
                    repo: input.name,
                    full_name,
                ...
                const failurePayload: ListRepoOrganizationAgentSecretsFailure = {
                    success: false,
