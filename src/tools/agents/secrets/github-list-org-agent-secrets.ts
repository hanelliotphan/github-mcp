...
export function registerGithubListOrgAgentSecretsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_org_agent_secrets",
        "List GitHub agent secrets in an organization without revealing their values (GET /orgs/{org}/agents/secrets). " +
            "Returns **`total_count`** and **`secrets`** (name, timestamps, visibility). " +
            ...
            "See [List organization secrets](https://docs.github.com/en/rest/agents/secrets?apiVersion=2026-03-10#list-organization-secrets).",
        {
            org: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllPageLinkPages({
                        ...
                        fetchPage: async (page, pp) => {
                            const response = await octokit.request("GET /orgs/{org}/agents/secrets", {
                                org: input.org,
                                per_page: pp,
                                page
                            });
                    ...
                    const successPayload: ListOrgAgentSecretsSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Organization agent secrets partially listed (${result.pagesFetched} pages, ${secrets.length} rows); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Organization agent secrets listed successfully (${result.pagesFetched} pages, ${secrets.length} rows).`
                              : "Organization agent secrets listed successfully.",
                        org: input.org,
                ...
                const successPayload: ListOrgAgentSecretsSuccess = {
                    success: true,
                    message: "Organization agent secrets listed successfully.",
                    org: input.org,
                ...
                const failurePayload: ListOrgAgentSecretsFailure = {
                    success: false,
