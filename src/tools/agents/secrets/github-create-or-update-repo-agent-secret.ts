...
export function registerGithubCreateOrUpdateRepoAgentSecretTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_create_or_update_repo_agent_secret",
        "Create or update a repository GitHub agent secret (PUT /repos/{owner}/{repo}/agents/secrets/{secret_name}). " +
            ...
            "See [Create or update a repository secret](https://docs.github.com/en/rest/agents/secrets?apiVersion=2026-03-10#create-or-update-a-repository-secret).",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                const keyResponse = await octokit.request("GET /repos/{owner}/{repo}/agents/secrets/public-key", {
                    owner: input.owner,
                    repo: input.name
                });
                const key = keyResponse.data as { key_id: string; key: string };
                const encryptedValue = await encryptSecretValue(input.value, key.key);
                const response = await octokit.request("PUT /repos/{owner}/{repo}/agents/secrets/{secret_name}", {
                    owner: input.owner,
                    repo: input.name,
                ...
                const successPayload: CreateOrUpdateRepoAgentSecretSuccess = {
                    success: true,
                    message: created
                        ? "Repository agent secret created successfully."
                        : "Repository agent secret updated successfully.",
                ...
                const failurePayload: CreateOrUpdateRepoAgentSecretFailure = {
                    success: false,
