...
import { z } from "zod";
import type { ListRepoDependabotAlertsSuccess, ListRepoDependabotAlertsFailure } from "../../../types.js";
...
export function registerGithubListRepoDependabotAlertsTool(server: McpServer, octokit: Octokit): void {
    server.tool("github_list_repo_dependabot_alerts", "List Dependabot alerts for a repository (GET /repos/{owner}/{repo}/dependabot/alerts). Optional filters including manifest; cursor pagination via before/after. Classic tokens need security_events. See [List Dependabot alerts for a repository](https://docs.github.com/en/rest/dependabot/alerts?apiVersion=2026-03-10#list-dependabot-alerts-for-a-repository).", {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllCursorLinkPages({
                        ...
                        fetchChunk: async (cursors) => {
                            const response = await octokit.rest.dependabot.listAlertsForRepo({ owner: input.owner, repo: input.name, per_page: perPage, ...filters, ...(cursors.after ? { after: cursors.after } : {}), ...(cursors.before ? { before: cursors.before } : {}) });
                    ...
                    const successPayload: ListRepoDependabotAlertsSuccess = {
                        success: true,
                ...
                const response = await octokit.rest.dependabot.listAlertsForRepo({ owner: input.owner, repo: input.name, per_page: perPage, ...filters, ...(input.after !== undefined ? { after: input.after } : {}), ...(input.before !== undefined ? { before: input.before } : {}) });
                ...
                const failurePayload: ListRepoDependabotAlertsFailure = { success: false, error: mapGitHubError(error), request_id: getRequestId((error as { response?: { headers?: Record<string, unknown> } }).response?.headers?.["x-github-request-id"]) };
