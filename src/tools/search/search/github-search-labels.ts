...
export function registerGithubSearchLabelsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_search_labels",
        "Search labels in a repository (GET /search/labels). " +
            "Requires **`repository_id`**. Query does not accept qualifiers. " +
            ...
            "See [Search labels](https://docs.github.com/en/rest/search/search?apiVersion=2026-03-10#search-labels).",
        {
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const successPayload: SearchLabelsSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Label search results partially listed (${result.pagesFetched} pages, ${items.length} items); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Label search results retrieved successfully (${result.pagesFetched} pages, ${items.length} items).`
                              : "Label search results retrieved successfully.",
                        q: input.q,
                        repository_id: input.repository_id,
                ...
                const successPayload: SearchLabelsSuccess = {
                    success: true,
                    message: "Label search results retrieved successfully.",
                    q: input.q,
                    repository_id: input.repository_id,
                ...
                const failurePayload: SearchLabelsFailure = {
                    success: false,
