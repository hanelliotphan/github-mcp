...
export function registerGithubSearchTopicsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        ...
        "Search topics (GET /search/topics). " +
            ...
            "See [Search topics](https://docs.github.com/en/rest/search/search?apiVersion=2026-03-10#search-topics).",
        {
            q: z.string().min(1),
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllPageLinkPages({
                        ...
                        fetchPage: async (page, pp) => {
                            const response = await octokit.rest.search.topics({
                                q: input.q,
                                per_page: pp,
                                page,
                    ...
                    const successPayload: SearchTopicsSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Topic search results partially listed (${result.pagesFetched} pages, ${items.length} items); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Topic search results retrieved successfully (${result.pagesFetched} pages, ${items.length} items).`
                              : "Topic search results retrieved successfully.",
                        q: input.q,
                ...
                const successPayload: SearchTopicsSuccess = {
                    success: true,
                    message: "Topic search results retrieved successfully.",
                    q: input.q,
                ...
                const failurePayload: SearchTopicsFailure = {
                    success: false,
