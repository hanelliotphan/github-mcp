...
export function registerGithubSearchCodeTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_search_code",
        "Search code (GET /search/code). " +
            "Requires authentication; rate limited to **10** requests/minute. `sort`/`order` are closing down. " +
            ...
            "See [Search code](https://docs.github.com/en/rest/search/search?apiVersion=2026-03-10#search-code).",
        {
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const successPayload: SearchCodeSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Code search results partially listed (${result.pagesFetched} pages, ${items.length} items); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Code search results retrieved successfully (${result.pagesFetched} pages, ${items.length} items).`
                              : "Code search results retrieved successfully.",
                        q: input.q,
                ...
                const successPayload: SearchCodeSuccess = {
                    success: true,
                    message: "Code search results retrieved successfully.",
                    q: input.q,
                ...
                const failurePayload: SearchCodeFailure = {
                    success: false,
