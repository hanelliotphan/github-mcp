...
export function registerGithubSearchCommitsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        ...
        "Search commits (GET /search/commits). " +
            ...
            "See [Search commits](https://docs.github.com/en/rest/search/search?apiVersion=2026-03-10#search-commits).",
        {
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const successPayload: SearchCommitsSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Commit search results partially listed (${result.pagesFetched} pages, ${items.length} items); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Commit search results retrieved successfully (${result.pagesFetched} pages, ${items.length} items).`
                              : "Commit search results retrieved successfully.",
                        q: input.q,
                ...
                const successPayload: SearchCommitsSuccess = {
                    success: true,
                    message: "Commit search results retrieved successfully.",
                    q: input.q,
                ...
                const failurePayload: SearchCommitsFailure = {
                    success: false,
