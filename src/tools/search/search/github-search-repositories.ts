...
export function registerGithubSearchRepositoriesTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        ...
        "Search repositories (GET /search/repositories). " +
            ...
            "See [Search repositories](https://docs.github.com/en/rest/search/search?apiVersion=2026-03-10#search-repositories).",
        {
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const successPayload: SearchRepositoriesSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Repository search results partially listed (${result.pagesFetched} pages, ${items.length} items); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Repository search results retrieved successfully (${result.pagesFetched} pages, ${items.length} items).`
                              : "Repository search results retrieved successfully.",
                        q: input.q,
                ...
                const successPayload: SearchRepositoriesSuccess = {
                    success: true,
                    message: "Repository search results retrieved successfully.",
                    q: input.q,
                ...
                const failurePayload: SearchRepositoriesFailure = {
                    success: false,
