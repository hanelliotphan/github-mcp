...
export function registerGithubListConcurrencyGroupsForRepoTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_concurrency_groups_for_repo",
        "List active GitHub Actions concurrency groups for a repository (GET /repos/{owner}/{repo}/actions/concurrency_groups). " +
            "Returns **`total_count`** and **`concurrency_groups`** rows (`group_name`, `group_url`, `last_acquired_at`). " +
            "Uses cursor pagination via the **`after`** cursor from the `Link` header. " +
            ...
            "See [List concurrency groups for a repository](https://docs.github.com/en/rest/actions/concurrency-groups?apiVersion=2026-03-10#list-concurrency-groups-for-a-repository).",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                ...
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListConcurrencyGroupsForRepoSuccess = {
                        success: true,
                ...
                const { rows, linkHeader, requestId } = await fetchChunk({ after: input.after });
                const successPayload: ListConcurrencyGroupsForRepoSuccess = {
                    success: true,
                ...
                const failurePayload: ListConcurrencyGroupsForRepoFailure = {
                    success: false,
