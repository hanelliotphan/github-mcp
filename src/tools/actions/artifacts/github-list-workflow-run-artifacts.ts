...
export function registerGithubListWorkflowRunArtifactsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_workflow_run_artifacts",
        "List GitHub Actions artifacts for a workflow run (GET /repos/{owner}/{repo}/actions/runs/{run_id}/artifacts). " +
            ...
            "Optional **`artifact_name`** filters to artifacts with that exact name; **`direction`** (`asc` or `desc`, default `desc`) sorts the results. " +
            ...
            "See [List workflow run artifacts](https://docs.github.com/en/rest/actions/artifacts?apiVersion=2026-03-10#list-workflow-run-artifacts).",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListWorkflowRunArtifactsSuccess = {
                        success: true,
                ...
                const failurePayload: ListWorkflowRunArtifactsFailure = {
                    success: false,
