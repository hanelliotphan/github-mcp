...
export function registerGithubListRepoOrganizationActionsVariablesTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_repo_organization_actions_variables",
        "List organization GitHub Actions variables shared with a repository (GET /repos/{owner}/{repo}/actions/organization-variables). " +
            ...
            "See [List repository organization variables](https://docs.github.com/en/rest/actions/variables?apiVersion=2026-03-10#list-repository-organization-variables).",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllPageLinkPages({
                        ...
                        fetchPage: async (page, pp) => {
                            const response = await octokit.rest.actions.listRepoOrganizationVariables({
                                owner: input.owner,
                                repo: input.name,
                                per_page: pp,
                                page
                    ...
                    const successPayload: ListRepoOrganizationActionsVariablesSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Repository organization variables partially listed (${result.pagesFetched} pages, ${variables.length} rows); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Repository organization variables listed successfully (${result.pagesFetched} pages, ${variables.length} rows).`
                              : "Repository organization variables listed successfully.",
                        owner: input.owner,
                        repo: input.name,
                        full_name,
                ...
                const successPayload: ListRepoOrganizationActionsVariablesSuccess = {
                    success: true,
                    message: "Repository organization variables listed successfully.",
                    owner: input.owner,
                    repo: input.name,
                    full_name,
                ...
                const failurePayload: ListRepoOrganizationActionsVariablesFailure = {
                    success: false,
