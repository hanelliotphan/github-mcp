...
export function registerGithubSetSelectedReposForOrgActionsVariableTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_set_selected_repos_for_org_actions_variable",
        "Replace the full set of repositories that can access a `selected`-visibility organization variable (PUT /orgs/{org}/actions/variables/{name}/repositories). " +
            ...
            "See [Set selected repositories for an organization variable](https://docs.github.com/en/rest/actions/variables?apiVersion=2026-03-10#set-selected-repositories-for-an-organization-variable).",
        {
            org: z
                .string()
                .min(1)
        ...
        async (input) => {
            try {
                ...
                const successPayload: SetSelectedReposForOrgActionsVariableSuccess = {
                    success: true,
                ...
                const failurePayload: SetSelectedReposForOrgActionsVariableFailure = {
                    success: false,
