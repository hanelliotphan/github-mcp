...
export function registerGithubListRepoActionsVariablesTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_repo_actions_variables",
        "List repository GitHub Actions variables (GET /repos/{owner}/{repo}/actions/variables). " +
            ...
            "See [List repository variables](https://docs.github.com/en/rest/actions/variables?apiVersion=2026-03-10#list-repository-variables).",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListRepoActionsVariablesSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Repository variables partially listed (${result.pagesFetched} pages, ${variables.length} rows); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Repository variables listed successfully (${result.pagesFetched} pages, ${variables.length} rows).`
                              : "Repository variables listed successfully.",
                        owner: input.owner,
                        repo: input.name,
                        full_name,
                ...
                const successPayload: ListRepoActionsVariablesSuccess = {
                    success: true,
                    message: "Repository variables listed successfully.",
                    owner: input.owner,
                    repo: input.name,
                    full_name,
                ...
                const failurePayload: ListRepoActionsVariablesFailure = {
                    success: false,
