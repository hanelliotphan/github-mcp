...
export function registerGithubListSelectedRepositoriesEnabledGithubActionsOrganizationTool(
    server: McpServer,
    octokit: Octokit
): void {
    server.tool(
        "github_list_selected_repositories_enabled_github_actions_organization",
        "List selected repositories enabled for GitHub Actions in an organization (GET /orgs/{org}/actions/permissions/repositories). " +
            "Applies when the org permissions policy is set to `selected`. Returns **`total_count`** and **`repositories`**. " +
            ...
            "See [List selected repositories enabled for GitHub Actions in an organization](https://docs.github.com/en/rest/actions/permissions?apiVersion=2026-03-10#list-selected-repositories-enabled-for-github-actions-in-an-organization).",
        {
            org: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllPageLinkPages({
                        ...
                        fetchPage: async (page, pp) => {
                            const response =
                                await octokit.rest.actions.listSelectedRepositoriesEnabledGithubActionsOrganization(
                                    {
                                        org: input.org,
                                        per_page: pp,
                                        page
                    ...
                    const successPayload: ListSelectedRepositoriesEnabledGithubActionsOrganizationSuccess = {
                        success: true,
                ...
                const failurePayload: ListSelectedRepositoriesEnabledGithubActionsOrganizationFailure = {
                    success: false,
