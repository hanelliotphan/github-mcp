...
export function registerGithubListJobsForWorkflowRunTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_jobs_for_workflow_run",
        "List jobs for a workflow run (GET /repos/{owner}/{repo}/actions/runs/{run_id}/jobs). " +
            "Returns **`total_count`** and **`jobs`**. Use **`filter`** = `latest` (default; most recent execution) or `all` (all executions). " +
            ...
            "See [List jobs for a workflow run](https://docs.github.com/en/rest/actions/workflow-jobs?apiVersion=2026-03-10#list-jobs-for-a-workflow-run).",
        {
            owner: z
                .string()
                .min(1)
            ...
            filter: z
                .enum(["latest", "all"])
                .optional()
                .describe("Filter jobs by completed_at. `latest` (default) returns jobs from the most recent run execution; `all` includes old executions."),
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListJobsForWorkflowRunSuccess = {
                        success: true,
                ...
                const failurePayload: ListJobsForWorkflowRunFailure = {
                    success: false,
