...
export function registerGithubListLabelsForSelfHostedRunnerForRepoTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_labels_for_self_hosted_runner_for_repo",
        "List all labels for a self-hosted runner configured in a repository (GET /repos/{owner}/{repo}/actions/runners/{runner_id}/labels). " +
            ...
            "See [List labels for a self-hosted runner for a repository](https://docs.github.com/en/rest/actions/self-hosted-runners?apiVersion=2026-03-10#list-labels-for-a-self-hosted-runner-for-a-repository).",
        {
        ...
        async (input) => {
            try {
                const response = await octokit.rest.actions.listLabelsForSelfHostedRunnerForRepo({
                    owner: input.owner,
                    repo: input.name,
                ...
                const successPayload: ListLabelsForSelfHostedRunnerForRepoSuccess = {
                    success: true,
                ...
                const failurePayload: ListLabelsForSelfHostedRunnerForRepoFailure = {
                    success: false,
