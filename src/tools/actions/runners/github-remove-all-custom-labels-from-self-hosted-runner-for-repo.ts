...
export function registerGithubRemoveAllCustomLabelsFromSelfHostedRunnerForRepoTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_remove_all_custom_labels_from_self_hosted_runner_for_repo",
        "Remove all custom labels from a self-hosted runner configured in a repository (DELETE /repos/{owner}/{repo}/actions/runners/{runner_id}/labels). " +
            "Returns the remaining read-only labels for the runner. " +
            "The authenticated user must have **admin** access to the repository; classic OAuth apps and PATs need the **`repo`** scope. " +
            "See [Remove all custom labels from a self-hosted runner for a repository](https://docs.github.com/en/rest/actions/self-hosted-runners?apiVersion=2026-03-10#remove-all-custom-labels-from-a-self-hosted-runner-for-a-repository).",
        {
        ...
        async (input) => {
            try {
                const response = await octokit.rest.actions.removeAllCustomLabelsFromSelfHostedRunnerForRepo({
                    owner: input.owner,
                    repo: input.name,
                ...
                const successPayload: RemoveAllCustomLabelsFromSelfHostedRunnerForRepoSuccess = {
                    success: true,
                    message: "All custom labels removed from runner successfully.",
                ...
                const failurePayload: RemoveAllCustomLabelsFromSelfHostedRunnerForRepoFailure = {
                    success: false,
