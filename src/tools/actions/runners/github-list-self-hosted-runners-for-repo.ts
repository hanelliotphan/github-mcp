...
export function registerGithubListSelfHostedRunnersForRepoTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_self_hosted_runners_for_repo",
        "List self-hosted runners configured in a repository (GET /repos/{owner}/{repo}/actions/runners). " +
            ...
            "See [List self-hosted runners for a repository](https://docs.github.com/en/rest/actions/self-hosted-runners?apiVersion=2026-03-10#list-self-hosted-runners-for-a-repository).",
        {
            ...
            runner_name: z.string().optional().describe("The name of a self-hosted runner to filter by."),
        ...
        async (input) => {
            ...
            const nameFilter = input.runner_name ? { name: input.runner_name } : {};
            try {
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListSelfHostedRunnersForRepoSuccess = {
                        success: true,
                ...
                const failurePayload: ListSelfHostedRunnersForRepoFailure = {
                    success: false,
