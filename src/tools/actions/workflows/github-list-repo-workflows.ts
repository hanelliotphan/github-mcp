...
function parseBody(data: unknown): { total_count: number; rows: unknown[] } {
    if (data && typeof data === "object" && "workflows" in data) {
        ...
        const rows = Array.isArray(o.workflows) ? o.workflows : [];
...
export function registerGithubListRepoWorkflowsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_repo_workflows",
        "List the workflows in a repository (GET /repos/{owner}/{repo}/actions/workflows). " +
            "Returns **`total_count`** and **`workflows`** (each with id, name, path, state, and URLs). " +
            ...
            "See [List repository workflows](https://docs.github.com/en/rest/actions/workflows?apiVersion=2026-03-10#list-repository-workflows).",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const workflows = toPlain(result.rows);
                    const successPayload: ListRepoWorkflowsSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Workflows partially listed (${result.pagesFetched} pages, ${workflows.length} rows); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Workflows listed successfully (${result.pagesFetched} pages, ${workflows.length} rows).`
                              : "Workflows listed successfully.",
                        owner: input.owner,
                        repo: input.name,
                        full_name,
                        total_count: firstTotalCount ?? workflows.length,
                        workflows,
                ...
                const failurePayload: ListRepoWorkflowsFailure = {
                    success: false,
