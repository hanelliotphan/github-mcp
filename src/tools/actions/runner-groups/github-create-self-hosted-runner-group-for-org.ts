...
export function registerGithubCreateSelfHostedRunnerGroupForOrgTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_create_self_hosted_runner_group_for_org",
        "Create a self-hosted runner group for an organization (POST /orgs/{org}/actions/runner-groups). " +
            "Provide **`name`** and optionally `visibility` (`selected`|`all`|`private`), `selected_repository_ids`, `runners`, `allows_public_repositories`, `restricted_to_workflows`, `selected_workflows`, and `network_configuration_id`. " +
            "Classic OAuth apps and PATs need the **`admin:org`** scope. Returns HTTP **201** with the created group. " +
            "See [Create a self-hosted runner group for an organization](https://docs.github.com/en/rest/actions/self-hosted-runner-groups?apiVersion=2026-03-10#create-a-self-hosted-runner-group-for-an-organization).",
        {
            org: z
                .string()
                .min(1)
            ...
            runners: z.array(z.number().int().positive()).optional(),
        ...
        async (input) => {
            try {
                const response = await octokit.request("POST /orgs/{org}/actions/runner-groups", {
                    org: input.org,
                    name: input.name,
                ...
                const successPayload: CreateSelfHostedRunnerGroupForOrgSuccess = {
                    success: true,
                    message: "Runner group created successfully.",
                ...
                const failurePayload: CreateSelfHostedRunnerGroupForOrgFailure = {
                    success: false,
