...
export function registerGithubReRunWorkflowTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_re_run_workflow",
        "Re-run an entire workflow run by its id (POST /repos/{owner}/{repo}/actions/runs/{run_id}/rerun). " +
            ...
            "See [Re-run a workflow](https://docs.github.com/en/rest/actions/workflow-runs?apiVersion=2026-03-10#re-run-a-workflow).",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                ...
                const successPayload: ReRunWorkflowSuccess = {
                    success: true,
                    message: "Workflow re-run requested successfully.",
                ...
                const failurePayload: ReRunWorkflowFailure = {
                    success: false,
