...
function unexpectedRedirectError(status: number): ErrorEnvelope {
    return {
        ...
        message: `Expected HTTP 302 with a Location header for the workflow run logs redirect; got status ${status}.`,
        hint: "The run or its logs may no longer be available. Otherwise GitHub may have changed the logs API.",
        retryable: false
    };
}

export function registerGithubDownloadWorkflowRunLogsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_download_workflow_run_logs",
        "Get a temporary download URL for a workflow run's log archive (GET /repos/{owner}/{repo}/actions/runs/{run_id}/logs). " +
            ...
            "See [Download workflow run logs](https://docs.github.com/en/rest/actions/workflow-runs?apiVersion=2026-03-10#download-workflow-run-logs).",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                ...
                if (response.status !== 302 || !logsDownloadUrl) {
                    const failurePayload: DownloadWorkflowRunLogsFailure = {
                        success: false,
                ...
                const successPayload: DownloadWorkflowRunLogsSuccess = {
                    success: true,
                    message: "Workflow run logs redirect URL retrieved successfully. Follow the URL with GET (e.g. curl -L) to download; it expires after 1 minute.",
