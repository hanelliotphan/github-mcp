...
export function registerGithubListRepoActionsSecretsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_repo_actions_secrets",
        "List GitHub Actions secrets in a repository, without revealing their values (GET /repos/{owner}/{repo}/actions/secrets). " +
            ...
            "See [List repository secrets](https://docs.github.com/en/rest/actions/secrets?apiVersion=2026-03-10#list-repository-secrets).",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListRepoActionsSecretsSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Repository secrets partially listed (${result.pagesFetched} pages, ${secrets.length} rows); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Repository secrets listed successfully (${result.pagesFetched} pages, ${secrets.length} rows).`
                              : "Repository secrets listed successfully.",
                        owner: input.owner,
                        repo: input.name,
                        full_name,
                ...
                const successPayload: ListRepoActionsSecretsSuccess = {
                    success: true,
                    message: "Repository secrets listed successfully.",
                    owner: input.owner,
                    repo: input.name,
                    full_name,
                ...
                const failurePayload: ListRepoActionsSecretsFailure = {
                    success: false,
