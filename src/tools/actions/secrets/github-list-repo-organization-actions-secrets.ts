...
export function registerGithubListRepoOrganizationActionsSecretsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_repo_organization_actions_secrets",
        "List organization GitHub Actions secrets shared with a repository, without revealing their values (GET /repos/{owner}/{repo}/actions/organization-secrets). " +
            ...
            "See [List repository organization secrets](https://docs.github.com/en/rest/actions/secrets?apiVersion=2026-03-10#list-repository-organization-secrets).",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllPageLinkPages({
                        ...
                        fetchPage: async (page, pp) => {
                            const response = await octokit.rest.actions.listRepoOrganizationSecrets({
                                owner: input.owner,
                                repo: input.name,
                                per_page: pp,
                                page
                    ...
                    const successPayload: ListRepoOrganizationActionsSecretsSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Repository organization secrets partially listed (${result.pagesFetched} pages, ${secrets.length} rows); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Repository organization secrets listed successfully (${result.pagesFetched} pages, ${secrets.length} rows).`
                              : "Repository organization secrets listed successfully.",
                        owner: input.owner,
                        repo: input.name,
                        full_name,
                ...
                const successPayload: ListRepoOrganizationActionsSecretsSuccess = {
                    success: true,
                    message: "Repository organization secrets listed successfully.",
                    owner: input.owner,
                    repo: input.name,
                    full_name,
                ...
                const failurePayload: ListRepoOrganizationActionsSecretsFailure = {
                    success: false,
