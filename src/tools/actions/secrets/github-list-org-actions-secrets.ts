...
export function registerGithubListOrgActionsSecretsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_org_actions_secrets",
        "List GitHub Actions secrets in an organization without revealing their values (GET /orgs/{org}/actions/secrets). " +
            ...
            "See [List organization secrets](https://docs.github.com/en/rest/actions/secrets?apiVersion=2026-03-10#list-organization-secrets).",
        {
            org: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllPageLinkPages({
                        ...
                        fetchPage: async (page, pp) => {
                            const response = await octokit.rest.actions.listOrgSecrets({
                                org: input.org,
                                per_page: pp,
                                page
                            });
                    ...
                    const successPayload: ListOrgActionsSecretsSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Organization secrets partially listed (${result.pagesFetched} pages, ${secrets.length} rows); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Organization secrets listed successfully (${result.pagesFetched} pages, ${secrets.length} rows).`
                              : "Organization secrets listed successfully.",
                        org: input.org,
                ...
                const successPayload: ListOrgActionsSecretsSuccess = {
                    success: true,
                    message: "Organization secrets listed successfully.",
                    org: input.org,
                ...
                const failurePayload: ListOrgActionsSecretsFailure = {
                    success: false,
