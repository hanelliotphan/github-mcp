...
export function registerGithubListSelectedReposForOrgActionsSecretTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_selected_repos_for_org_actions_secret",
        "List repositories that can access a `selected`-visibility organization GitHub Actions secret (GET /orgs/{org}/actions/secrets/{secret_name}/repositories). " +
            ...
            "See [List selected repositories for an organization secret](https://docs.github.com/en/rest/actions/secrets?apiVersion=2026-03-10#list-selected-repositories-for-an-organization-secret).",
        {
            org: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllPageLinkPages({
                        ...
                        fetchPage: async (page, pp) => {
                            const response = await octokit.rest.actions.listSelectedReposForOrgSecret({
                                org: input.org,
                    ...
                    const successPayload: ListSelectedReposForOrgActionsSecretSuccess = {
                        success: true,
                ...
                const failurePayload: ListSelectedReposForOrgActionsSecretFailure = {
                    success: false,
