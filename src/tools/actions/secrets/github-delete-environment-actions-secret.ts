...
export function registerGithubDeleteEnvironmentActionsSecretTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_delete_environment_actions_secret",
        "Delete an environment GitHub Actions secret by name (DELETE /repos/{owner}/{repo}/environments/{environment_name}/secrets/{secret_name}). " +
            "The authenticated user must have collaborator access; classic OAuth apps and PATs need the **`repo`** scope. Success is HTTP **204**. " +
            "See [Delete an environment secret](https://docs.github.com/en/rest/actions/secrets?apiVersion=2026-03-10#delete-an-environment-secret).",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                ...
                const successPayload: DeleteEnvironmentActionsSecretSuccess = {
                    success: true,
                ...
                const failurePayload: DeleteEnvironmentActionsSecretFailure = {
                    success: false,
