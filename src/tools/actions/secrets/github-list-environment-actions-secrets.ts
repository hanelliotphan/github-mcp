...
export function registerGithubListEnvironmentActionsSecretsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_environment_actions_secrets",
        "List GitHub Actions secrets in a repository environment, without revealing their values (GET /repos/{owner}/{repo}/environments/{environment_name}/secrets). " +
            ...
            "See [List environment secrets](https://docs.github.com/en/rest/actions/secrets?apiVersion=2026-03-10#list-environment-secrets).",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListEnvironmentActionsSecretsSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Environment secrets partially listed (${result.pagesFetched} pages, ${secrets.length} rows); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Environment secrets listed successfully (${result.pagesFetched} pages, ${secrets.length} rows).`
                              : "Environment secrets listed successfully.",
                        owner: input.owner,
                        repo: input.name,
                        full_name,
                        environment_name: input.environment_name,
                ...
                const successPayload: ListEnvironmentActionsSecretsSuccess = {
                    success: true,
                    message: "Environment secrets listed successfully.",
                    owner: input.owner,
                    repo: input.name,
                    full_name,
                    environment_name: input.environment_name,
                ...
                const failurePayload: ListEnvironmentActionsSecretsFailure = {
                    success: false,
