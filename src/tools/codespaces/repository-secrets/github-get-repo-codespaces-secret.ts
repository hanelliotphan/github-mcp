...
export function registerGithubGetRepoCodespacesSecretTool(server: McpServer, octokit: Octokit): void {
    server.tool("github_get_repo_codespaces_secret", "Get repository codespaces secret metadata. See GitHub REST repository secrets.", { owner: z
                .string()
                .min(1)
                .max(39)
        ...
        try {
            ...
            return textAndData({ success: false, error: mapGitHubError(error), request_id: getRequestId((error as { response?: { headers?: Record<string, unknown> } }).response?.headers?.["x-github-request-id"]) } satisfies GetRepoCodespacesSecretFailure);
        }
    });
}