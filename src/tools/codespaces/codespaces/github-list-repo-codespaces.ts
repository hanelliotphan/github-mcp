...
export function registerGithubListRepoCodespacesTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_repo_codespaces",
        "List codespaces in a repository for the authenticated user (GET /repos/{owner}/{repo}/codespaces). Returns total_count and codespaces. Classic tokens need the codespace scope. See GitHub REST Codespaces.",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListRepoCodespacesSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Repository codespaces partially listed (${result.pagesFetched} pages, ${codespaces.length} rows); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Repository codespaces listed successfully (${result.pagesFetched} pages, ${codespaces.length} rows).`
                              : "Repository codespaces listed successfully.",
                        owner: input.owner,
                        repo: input.name,
                        full_name,
                ...
                const successPayload: ListRepoCodespacesSuccess = {
                    success: true,
                    message: "Repository codespaces listed successfully.",
                    owner: input.owner,
                        repo: input.name,
                        full_name,
                ...
                const failurePayload: ListRepoCodespacesFailure = {
                    success: false,
