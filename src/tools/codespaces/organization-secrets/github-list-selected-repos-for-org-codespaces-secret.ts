...
export function registerGithubListSelectedReposForOrgCodespacesSecretTool(server: McpServer, octokit: Octokit): void {
    server.tool("github_list_selected_repos_for_org_codespaces_secret", "List selected repositories for organization codespaces secret. See GitHub REST organization secrets.", { org: z
                .string()
                .min(1)
                .max(39)
        ...
        try {
            if (input.all_pages === true) {
                ...
                const result = await fetchAllPageLinkPages({ perPage, maxPages, fetchPage: async (page, pp) => {
                    const response = await octokit.request("GET /orgs/{org}/codespaces/secrets/{secret_name}/repositories", { org: input.org, secret_name: input.secret_name, per_page: pp, page });
                ...
                return textAndData({ success: true, message: "Selected repositories listed successfully.", org: input.org, secret_name: input.secret_name, total_count: firstTotalCount ?? repositories.length, repositories, pagination: result.responsePagination, request_id: result.lastRequestId, page: result.lastPage, per_page: perPage, pages_fetched: result.pagesFetched, truncated: result.truncated || undefined } satisfies ListSelectedReposForOrgCodespacesSecretSuccess);
            }
            const page = input.page ?? 1;
            const response = await octokit.request("GET /orgs/{org}/codespaces/secrets/{secret_name}/repositories", { org: input.org, secret_name: input.secret_name, per_page: perPage, page });
            const parsed = parseBody(response.data);
            return textAndData({ success: true, message: "Selected repositories listed successfully.", org: input.org, secret_name: input.secret_name, total_count: parsed.total_count, repositories: toPlain(parsed.rows), pagination: parseGitHubPageLinkPagination(getLinkHeaderFromResponse(response.headers as { link?: string; Link?: string })), request_id: getRequestId(response.headers["x-github-request-id"]), page, per_page: perPage, pages_fetched: 1 } satisfies ListSelectedReposForOrgCodespacesSecretSuccess);
        } catch (error: unknown) {
            return textAndData({ success: false, error: mapGitHubError(error), request_id: getRequestId((error as { response?: { headers?: Record<string, unknown> } }).response?.headers?.["x-github-request-id"]) } satisfies ListSelectedReposForOrgCodespacesSecretFailure);
        }
    });
}