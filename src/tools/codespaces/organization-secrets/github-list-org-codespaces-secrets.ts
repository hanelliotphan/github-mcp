...
export function registerGithubListOrgCodespacesSecretsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_org_codespaces_secrets",
        "List organization codespaces secrets (GET /orgs/{org}/codespaces/secrets). See GitHub REST organization secrets.",
        {
            org: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllPageLinkPages({
                        ...
                        fetchPage: async (page, pp) => {
                            const response = await octokit.request("GET /orgs/{org}/codespaces/secrets", {
                                org: input.org,
                                per_page: pp,
                                page
                            });
                    ...
                    const successPayload: ListOrgCodespacesSecretsSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Organization codespaces secrets partially listed (${result.pagesFetched} pages, ${secrets.length} rows); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Organization codespaces secrets listed successfully (${result.pagesFetched} pages, ${secrets.length} rows).`
                              : "Organization codespaces secrets listed successfully.",
                        org: input.org,
                ...
                const successPayload: ListOrgCodespacesSecretsSuccess = {
                    success: true,
                    message: "Organization codespaces secrets listed successfully.",
                    org: input.org,
                ...
                const failurePayload: ListOrgCodespacesSecretsFailure = {
                    success: false,
