...
export function registerGithubListOrgCodespacesTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_org_codespaces",
        "List codespaces for an organization (GET /orgs/{org}/codespaces). See GitHub REST Codespaces organizations.",
        {
            org: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllPageLinkPages({
                        ...
                        fetchPage: async (page, pp) => {
                            const response = await octokit.request("GET /orgs/{org}/codespaces", {
                                org: input.org,
                                per_page: pp,
                                page
                            });
                    ...
                    const successPayload: ListOrgCodespacesSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Organization codespaces partially listed (${result.pagesFetched} pages, ${codespaces.length} rows); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Organization codespaces listed successfully (${result.pagesFetched} pages, ${codespaces.length} rows).`
                              : "Organization codespaces listed successfully.",
                        org: input.org,
                ...
                const successPayload: ListOrgCodespacesSuccess = {
                    success: true,
                    message: "Organization codespaces listed successfully.",
                    org: input.org,
                ...
                const failurePayload: ListOrgCodespacesFailure = {
                    success: false,
