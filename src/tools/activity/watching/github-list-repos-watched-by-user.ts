...
export function registerGithubListReposWatchedByUserTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_repos_watched_by_user",
        "List repositories a user is watching (GET /users/{username}/subscriptions). " +
            "Note: as of July 2026 this endpoint is deprecated—during the deprecation period it remains available but returns empty responses, and it will be removed later. " +
            "Use `per_page` (1–100, default **30** when omitted) and `page`; set `all_pages` to follow `next` links up to `max_pages` (default **100**). " +
            "See [List repositories watched by a user](https://docs.github.com/en/rest/activity/watching?apiVersion=2026-03-10#list-repositories-watched-by-a-user).",
        {
            username: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllPageLinkPages({
                        ...
                        fetchPage: async (page, pp) => {
                            const response = await octokit.rest.activity.listReposWatchedByUser({
                    ...
                    const successPayload: ListReposWatchedByUserSuccess = {
                        success: true,
                ...
                const failurePayload: ListReposWatchedByUserFailure = {
                    success: false,
