...
import type { CreateOrgCodeSecurityConfigurationSuccess, CreateOrgCodeSecurityConfigurationFailure } from "../../../types.js";
...
export function registerGithubCreateOrgCodeSecurityConfigurationTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_create_org_code_security_configuration",
        "Create a code security configuration for an organization (POST /orgs/{org}/code-security/configurations). Requires name and description. Classic tokens need write:org. See [Create a code security configuration](https://docs.github.com/en/rest/code-security/configurations?apiVersion=2026-03-10#create-a-code-security-configuration).",
        {
            org: z
                .string()
                .min(1)
        ...
        async (input) => {
            try {
                const response = await octokit.rest.codeSecurity.createConfiguration({
                    org: input.org,
                    name: input.name,
                ...
                const successPayload: CreateOrgCodeSecurityConfigurationSuccess = {
                    success: true,
                ...
                const failurePayload: CreateOrgCodeSecurityConfigurationFailure = {
                    success: false,
