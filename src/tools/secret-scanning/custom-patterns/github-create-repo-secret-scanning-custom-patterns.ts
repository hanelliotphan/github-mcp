...
import type { CreateRepoSecretScanningCustomPatternsSuccess, CreateRepoSecretScanningCustomPatternsFailure } from "../../../types.js";
...
export function registerGithubCreateRepoSecretScanningCustomPatternsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_create_repo_secret_scanning_custom_patterns",
        "Bulk create secret scanning custom patterns for a repository (POST /repos/{owner}/{repo}/secret-scanning/custom-patterns). See [Bulk create repository custom patterns](https://docs.github.com/en/rest/secret-scanning/custom-patterns?apiVersion=2026-03-10).",
        {
        ...
        async (input) => {
            try {
                const response = await octokit.request(
                    ...
                    { owner: input.owner, repo: input.name, patterns: input.patterns } as never
                );
                const requestId = getRequestId(response.headers["x-github-request-id"]);
                const successPayload: CreateRepoSecretScanningCustomPatternsSuccess = {
                    success: true,
                ...
                const failurePayload: CreateRepoSecretScanningCustomPatternsFailure = {
                    success: false,
