...
import type { ListEnterpriseSecretScanningBypassRequestsSuccess, ListEnterpriseSecretScanningBypassRequestsFailure } from "../../../types.js";
...
export function registerGithubListEnterpriseSecretScanningBypassRequestsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_enterprise_secret_scanning_bypass_requests",
        "List secret scanning bypass requests for an enterprise (GET /enterprises/{enterprise}/bypass-requests/secret-scanning). Classic tokens need security_events. See [List bypass requests for secret scanning for an enterprise](https://docs.github.com/en/enterprise-cloud@latest/rest/secret-scanning/delegated-bypass?apiVersion=2026-03-10#list-bypass-requests-for-secret-scanning-for-an-enterprise).",
        {
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllPageLinkPages({
                        ...
                        fetchPage: async (page, pp) => {
                            const response = await octokit.request(
                                ...
                                { enterprise: input.enterprise, ...filters, per_page: pp, page } as never
                            );
                            return {
                    ...
                    const successPayload: ListEnterpriseSecretScanningBypassRequestsSuccess = {
                        success: true,
                ...
                const response = await octokit.request("GET /enterprises/{enterprise}/bypass-requests/secret-scanning", {
                    enterprise: input.enterprise,
                    ...filters,
                    per_page: perPage,
                    page
                ...
                const failurePayload: ListEnterpriseSecretScanningBypassRequestsFailure = {
                    success: false,
