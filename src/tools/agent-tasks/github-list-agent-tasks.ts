...
export function registerGithubListAgentTasksTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_agent_tasks",
        "List Copilot cloud agent tasks for the authenticated user (GET /agents/tasks). " +
            ...
            "**`state`** (comma-separated task states), **`is_archived`**, **`since`** (ISO 8601). " +
            "Use `per_page` (1–100, default **30**) and `page`; set `all_pages` to follow `next` links up to `max_pages` (default **100**). " +
            "See [List tasks](https://docs.github.com/en/rest/agent-tasks/agent-tasks?apiVersion=2026-03-10#list-tasks).",
        {
        ...
        async (input) => {
            ...
            if (input.state) {
                ...
                for (const part of parts) {
                    ...
                    if (!parsed.success) {
                        const failurePayload: ListAgentTasksFailure = {
                            success: false,
                            error: {
                                status_code: 400,
                ...
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllPageLinkPages({
                        ...
                        fetchPage: async (page, pp) => {
                            const response = await octokit.request("GET /agents/tasks", {
                                ...filters,
                                per_page: pp,
                                page
                            });
                    ...
                    const successPayload: ListAgentTasksSuccess = {
                        success: true,
