...
function toPlainDeliveries(rows: unknown[]): RepoWebhookDeliveryItem[] {
    return rows.map((row) => JSON.parse(JSON.stringify(row)) as RepoWebhookDeliveryItem);
}

async function listWebhookDeliveriesRequest(
    octokit: Octokit,
    input: {
        owner: string;
        name: string;
    ...
    return octokit.request("GET /repos/{owner}/{repo}/hooks/{hook_id}/deliveries", {
        owner: input.owner,
        repo: input.name,
...
export function registerGithubListRepoWebhookDeliveriesTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_repo_webhook_deliveries",
        "List deliveries for a repository webhook (GET /repos/{owner}/{repo}/hooks/{hook_id}/deliveries). " +
            ...
            "See [List deliveries for a repository webhook](https://docs.github.com/en/rest/repos/webhooks?apiVersion=2026-03-10#list-deliveries-for-a-repository-webhook).",
        {
            owner: z
                .string()
                .min(1)
        ...
        async (input) => {
            ...
            try {
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllCursorQueryLinkPages({
                        ...
                        fetchChunk: async (cursor) => {
                            const response = await listWebhookDeliveriesRequest(octokit, {
                                owner: input.owner,
                                name: input.name,
                                hook_id: input.hook_id,
                                per_page: perPage,
                    ...
                    const successPayload: ListRepoWebhookDeliveriesSuccess = {
                        success: true,
                ...
                const failurePayload: ListRepoWebhookDeliveriesFailure = {
                    success: false,
