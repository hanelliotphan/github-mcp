...
export function registerGithubListMarketplaceAccountsForPlanTool(
    server: McpServer,
    octokit: Octokit
): void {
    server.tool(
        "github_list_marketplace_accounts_for_plan",
        "List user and organization accounts associated with a Marketplace plan " +
            "(GET /marketplace_listing/plans/{plan_id}/accounts), including free plans and pending changes. " +
            ...
            "See [List accounts for a plan](https://docs.github.com/en/rest/apps/marketplace?apiVersion=2026-03-10#list-accounts-for-a-plan).",
        {
        ...
        async (input) => {
            try {
                ...
                if (input.all_pages === true) {
                    ...
                    const result = await fetchAllPageLinkPages({
                        ...
                        fetchPage: async (page, pp) => {
                            const response = await octokit.rest.apps.listAccountsForPlan({
                    ...
                    const successPayload: ListMarketplaceAccountsForPlanSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Marketplace accounts partially listed (${result.pagesFetched} pages, ${accounts.length} rows); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Marketplace accounts listed successfully (${result.pagesFetched} pages, ${accounts.length} rows).`
                              : "Marketplace accounts listed successfully.",
                        plan_id: input.plan_id,
                        accounts,
                ...
                const successPayload: ListMarketplaceAccountsForPlanSuccess = {
                    success: true,
                    message: "Marketplace accounts listed successfully.",
                    plan_id: input.plan_id,
                    accounts,
                ...
                const failurePayload: ListMarketplaceAccountsForPlanFailure = {
                    success: false,
