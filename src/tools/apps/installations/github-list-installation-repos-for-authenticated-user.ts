...
export function registerGithubListInstallationReposForAuthenticatedUserTool(
    server: McpServer,
    octokit: Octokit
): void {
    server.tool(
        "github_list_installation_repos_for_authenticated_user",
        "List repositories accessible to the user for a GitHub App installation " +
            "(GET /user/installations/{installation_id}/repositories). " +
            "Returns **`total_count`** and **`repositories`** (and optional **`repository_selection`**). " +
            "Lists repositories the authenticated user has explicit **:read**, **:write**, or **:admin** permission to access for the installation. " +
            "You must authenticate with a **user access token**. " +
            ...
            "See [List repositories accessible to the user access token](https://docs.github.com/en/rest/apps/installations?apiVersion=2026-03-10#list-repositories-accessible-to-the-user-access-token).",
        {
        ...
        async (input) => {
            try {
                ...
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListInstallationReposForAuthenticatedUserSuccess = {
                        success: true,
                ...
                const failurePayload: ListInstallationReposForAuthenticatedUserFailure = {
                    success: false,
