...
- [`github_remove_public_org_membership_for_auth_user`](README.md#github_remove_public_org_membership_for_auth_user)
...
Calls [Remove public organization membership for the authenticated user](https://docs.github.com/en/rest/orgs/members?apiVersion=2026-03-10#remove-public-organization-membership-for-the-authenticated-user) (`DELETE /orgs/{org}/public_members/{username}`).
...
On success (**204**): **`org`**, **`username`**, **`http_status`**, **`request_id`**. On failure: **`error`**.

Does not remove private org membership; only hides public visibility unless the org enforces public membership by default per GitHub.
