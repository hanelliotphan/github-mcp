...
export function registerGithubGetOrgTimeStatsByUserTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        ...
        "Get API request counts **over time** for one **user** in an organization (GET /orgs/{org}/insights/api/time-stats/users/{user_id}). " +
            ...
            "See [Get time stats by user](https://docs.github.com/en/rest/orgs/api-insights?apiVersion=2026-03-10#get-time-stats-by-user).",
        {
            org: z
                .string()
                .min(1)
        ...
        async (input) => {
            try {
                ...
                const successPayload: GetTimeStatsByUserSuccess = {
                    success: true,
                    message: "Time stats for user retrieved successfully.",
                ...
                const failurePayload: GetTimeStatsByUserFailure = {
                    success: false,
