...
import type { CreateUserCopilotSpaceResourceSuccess, CreateUserCopilotSpaceResourceFailure } from "../../../types.js";
...
export function registerGithubCreateUserCopilotSpaceResourceTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_create_user_copilot_space_resource",
        "Create a resource for a user Copilot Space (POST /users/{username}/copilot-spaces/{space_number}/resources). Requires resource_type and metadata. Returns HTTP 201 or 200 for duplicate github_file. See [Create a resource for a Copilot Space for a user](https://docs.github.com/en/rest/copilot-spaces/resources?apiVersion=2026-03-10#create-a-resource-for-a-copilot-space-for-a-user).",
        {
        ...
        async (input) => {
            try {
                const response = await octokit.request("POST /users/{username}/copilot-spaces/{space_number}/resources", {
                ...
                const successPayload: CreateUserCopilotSpaceResourceSuccess = {
                    success: true,
                ...
                const failurePayload: CreateUserCopilotSpaceResourceFailure = {
                    success: false,
