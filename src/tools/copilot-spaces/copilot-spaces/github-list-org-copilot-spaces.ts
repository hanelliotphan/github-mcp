...
import type { ListOrgCopilotSpacesSuccess, ListOrgCopilotSpacesFailure } from "../../../types.js";
...
export function registerGithubListOrgCopilotSpacesTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_org_copilot_spaces",
        "List organization Copilot Spaces (GET /orgs/{org}/copilot-spaces). Cursor pagination via before/after/per_page; supports all_pages/max_pages. Returns spaces array. See [List organization Copilot Spaces](https://docs.github.com/en/rest/copilot-spaces/copilot-spaces?apiVersion=2026-03-10#list-organization-copilot-spaces).",
        {
        ...
        async (input) => {
            try {
                ...
                const fetchChunk = async (cursors: { after?: string; before?: string }) => {
                    const response = await octokit.request("GET /orgs/{org}/copilot-spaces", {
                        org: input.org,
                        per_page: perPage,
                ...
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListOrgCopilotSpacesSuccess = {
                        success: true,
                ...
                const failurePayload: ListOrgCopilotSpacesFailure = {
                    success: false,
