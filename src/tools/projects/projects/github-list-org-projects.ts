...
export function registerGithubListOrgProjectsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_org_projects",
        "List organization-owned projects (GET /orgs/{org}/projectsV2). " +
            ...
            "See [List projects for organization](https://docs.github.com/en/rest/projects/projects?apiVersion=2026-03-10#list-projects-for-organization).",
        {
            org: z
                .string()
                .min(1)
        ...
        async (input) => {
            try {
                ...
                const fetchChunk = async (cursors: { after?: string; before?: string }) => {
                    ...
                    const response = await octokit.rest.projects.listForOrg(params as never);
                    return {
                ...
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListOrgProjectsSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Organization projects partially listed (${result.pagesFetched} pages, ${projects.length} items); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Organization projects retrieved successfully (${result.pagesFetched} pages, ${projects.length} items).`
                ...
                const failurePayload: ListOrgProjectsFailure = {
                    success: false,
