...
export function registerGithubListUserProjectViewItemsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_user_project_view_items",
        "List items for a user project view (GET /users/{username}/projectsV2/{project_number}/views/{view_number}/items). " +
            ...
            "See [List items for a user project view](https://docs.github.com/en/rest/projects/items?apiVersion=2026-03-10#list-items-for-a-user-project-view).",
        {
            username: z
                .string()
                .min(1)
        ...
        async (input) => {
            try {
                ...
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListUserProjectViewItemsSuccess = {
                        success: true,
                        message: result.truncated
                            ? `User project view items partially listed (${result.pagesFetched} pages, ${items.length} items); more pages exist.`
                            : result.pagesFetched > 1
                              ? `User project view items retrieved successfully (${result.pagesFetched} pages, ${items.length} items).`
                              : "User project view items retrieved successfully.",
                        username: input.username,
                        project_number: input.project_number,
                        view_number: input.view_number,
                        items,
                ...
                const successPayload: ListUserProjectViewItemsSuccess = {
                    success: true,
                    message: "User project view items retrieved successfully.",
                    username: input.username,
                    project_number: input.project_number,
                    view_number: input.view_number,
                ...
                const failurePayload: ListUserProjectViewItemsFailure = {
                    success: false,
