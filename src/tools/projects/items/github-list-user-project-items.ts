...
export function registerGithubListUserProjectItemsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_user_project_items",
        "List items for a user-owned project (GET /users/{username}/projectsV2/{project_number}/items). " +
            ...
            "See [List items for a user owned project](https://docs.github.com/en/rest/projects/items?apiVersion=2026-03-10#list-items-for-a-user-owned-project).",
        {
            username: z
                .string()
                .min(1)
        ...
        async (input) => {
            try {
                ...
                const fetchChunk = async (cursors: { after?: string; before?: string }) => {
                    ...
                    const response = await octokit.rest.projects.listItemsForUser(params as never);
                    return {
                ...
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListUserProjectItemsSuccess = {
                        success: true,
                        message: result.truncated
                            ? `User project items partially listed (${result.pagesFetched} pages, ${items.length} items); more pages exist.`
                            : result.pagesFetched > 1
                              ? `User project items retrieved successfully (${result.pagesFetched} pages, ${items.length} items).`
                              : "User project items retrieved successfully.",
                        username: input.username,
                        project_number: input.project_number,
                        items,
                ...
                const successPayload: ListUserProjectItemsSuccess = {
                    success: true,
                    message: "User project items retrieved successfully.",
                    username: input.username,
                    project_number: input.project_number,
                ...
                const failurePayload: ListUserProjectItemsFailure = {
                    success: false,
