...
export function registerGithubListOrgProjectItemsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_org_project_items",
        "List items for an organization-owned project (GET /orgs/{org}/projectsV2/{project_number}/items). " +
            ...
            "See [List items for an organization owned project](https://docs.github.com/en/rest/projects/items?apiVersion=2026-03-10#list-items-for-an-organization-owned-project).",
        {
            org: z
                .string()
                .min(1)
        ...
        async (input) => {
            try {
                ...
                const fetchChunk = async (cursors: { after?: string; before?: string }) => {
                    ...
                    const response = await octokit.rest.projects.listItemsForOrg(params as never);
                    return {
                ...
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListOrgProjectItemsSuccess = {
                        success: true,
                        message: result.truncated
                            ? `Organization project items partially listed (${result.pagesFetched} pages, ${items.length} items); more pages exist.`
                            : result.pagesFetched > 1
                              ? `Organization project items retrieved successfully (${result.pagesFetched} pages, ${items.length} items).`
                              : "Organization project items retrieved successfully.",
                        org: input.org,
                        project_number: input.project_number,
                        items,
                ...
                const successPayload: ListOrgProjectItemsSuccess = {
                    success: true,
                    message: "Organization project items retrieved successfully.",
                    org: input.org,
                    project_number: input.project_number,
                ...
                const failurePayload: ListOrgProjectItemsFailure = {
                    success: false,
