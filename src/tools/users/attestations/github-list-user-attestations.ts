...
export function registerGithubListUserAttestationsTool(server: McpServer, octokit: Octokit): void {
    server.tool(
        "github_list_user_attestations",
        "List **artifact attestations** for an organization and subject digest (GET /users/{username}/attestations/{subject_digest}). " +
            ...
            "See [List attestations](https://docs.github.com/en/rest/users/attestations?apiVersion=2026-03-10#list-attestations).",
        {
            username: z
                .string()
                .min(1)
        ...
        async (input) => {
            try {
                ...
                const listAttestations = async (cursors: { after?: string; before?: string }) => {
                    ...
                    const response = await octokit.rest.users.listAttestations(params as never);
                ...
                if (input.all_pages === true) {
                    ...
                    const successPayload: ListUserAttestationsSuccess = {
                        success: true,
                        message: result.truncated
                            ? `User attestations partially listed (${result.pagesFetched} pages, ${attestations.length} items); more pages exist.`
                            : result.pagesFetched > 1
                              ? `User attestations retrieved successfully (${result.pagesFetched} pages, ${attestations.length} items).`
                ...
                const successPayload: ListUserAttestationsSuccess = {
                    success: true,
                    message: "User attestations retrieved successfully.",
                    username: input.username,
                    subject_digest: input.subject_digest,
                ...
                const failurePayload: ListUserAttestationsFailure = {
                    success: false,
